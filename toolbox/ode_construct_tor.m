function prob = ode_construct_tor(prob, tbid, data, sol)
%ODE_CONSTRUCT_tor   Append an instance of 'tor' to problem.
%
...
%
% PROB = ODE_CONSTRUCT_TOR(PROB, TBID, DATA, SOL)
%
...
prob = coco_add_func(prob, tbid, @tor, data, 'zero', ...
...
function [data, y, J] = tor(prob, data, u) %#ok<INUSL>
x0 = u(data.x0_idx);
x1 = u(data.x1_idx);
T0 = u(data.T0_idx);
T  = u(data.T_idx);
...
%   
%   tic
%   [~,Jnumer] = coco_ezDFDX('f(o,d,x)', prob, data, @tory, u);
%   toc
%   max(abs(J(:)-Jnumer(:)))
...
function [data, y] = tory(prob, data, u) %#ok<INUSL>
x0 = u(data.x0_idx);
x1 = u(data.x1_idx);
T0 = u(data.T0_idx);
T  = u(data.T_idx);
